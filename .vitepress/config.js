...
function applyDynamicHead(pageData, siteConfig) {
  ...
  const pageDesc = pageData.frontmatter.description || `VM汉化组是一个用爱发电专注于汉化MC整合包与地图的非盈利组织。\n我们制作的汉化及相关技术最终目的是使公众受益，将寻求高质量永久免费汉化补丁以造福玩家`
  const pageKeywords = pageData.frontmatter.keywords || 'VM汉化组,VM 汉化组,整合包,汉化,整合包汉化,MC汉化,永恒的MC,永恒的MC汉化,石头世界3'
  const pageCover = pageData.frontmatter.image || '/imgs/og_image.png'

  const metaTags = getMetaTags(pageUrl, pageTitle, pageDesc, pageKeywords, pageCover)
...
function applyDynamicTitleTemplate(pageData, siteConfig) {
  let titleTemplate = pageData.frontmatter.titleTemplate ?? siteConfig.userConfig.titleTemplate
  if (!titleTemplate) {
    const localeKey = pageData.relativePath.split('/', 1)[0] || 'root'
    const localeConfig = siteConfig.userConfig.locales?.[localeKey] ?? {}
    const templateMappings = localeConfig.themeConfig?.ui.title.templateMappings ?? []
...
function createConfigureFunction() {
  return {
    transformHead: ({ pageData, siteConfig }) => applyDynamicHead(pageData, siteConfig),
    transformPageData: (pageData, { siteConfig }) => applyDynamicTitleTemplate(pageData, siteConfig)
  };
}
